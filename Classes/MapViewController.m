...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    UITouch *touch = [[event allTouches] anyObject];
    if(gLastTouchTimestamp == touch.timestamp) return;
...
- (void) scrollViewDidEndZooming:(UIScrollView *)aScrollView withView:(UIView *)view atScale:(float)scale {
	...
	NSLog(@"scale: %f", scale);
	NSLog(@"bounds-origin: %f, %f -- bounds-size: %f, %f", view.bounds.origin.x, view.bounds.origin.y, view.bounds.size.width, view.bounds.size.height);
	NSLog(@"center: %f, %f", view.center.x, view.center.y);
	NSLog(@"content-offset: %f, %f", aScrollView.contentOffset.x, aScrollView.contentOffset.y);
}
